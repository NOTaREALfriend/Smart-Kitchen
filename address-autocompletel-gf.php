...
class bb_gf_address_autocomplete{
	...
	function bb_gf_advanced_settings( $position, $form_id ) {    
	      if ( $position == 25 ) { ?>
	        ...
	        <li class="autocomplete_SL_setting field_setting">
	            <label for="field_admin_label">
	                <?php _e("Use Single Line autofill", "gravityforms"); ?>
	                <?php gform_tooltip("form_field_autocomplete_SL_value") ?>
	            </label>
	            <input type="checkbox" id="field_autocomplete_SL_value" onclick="SetFieldProperty('autocompleteSLField', this.checked);" /> Enable
	        </li>
	        <li class="autocomplete_CC_setting field_setting">
	            <label for="field_admin_label">
	                <?php _e("Use City/Country autofill", "gravityforms"); ?>
	                <?php gform_tooltip("form_field_autocomplete_CC_value") ?>
	            </label>
	            <input type="checkbox" id="field_autocomplete_CC_value" onclick="SetFieldProperty('autocompleteCCField', this.checked);" /> Enable
	        </li>
	...
	function editor_script(){
	    ...
	    <script type='text/javascript'>
	        ...
	        fieldSettings["address"] += ", .autocomplete_SL_setting";
	        fieldSettings["address"] += ", .autocomplete_CC_setting";
	        ...
	        //binding to the load field settings event to initialize the checkbox
	            ...
	            jQuery("#field_autocomplete_SL_value").attr("checked", field["autocompleteSLField"] == true);
	            jQuery("#field_autocomplete_CC_value").attr("checked", field["autocompleteCCField"] == true);	            
		...
		foreach($form['fields'] as &$field){
			...
			if($field['type'] == 'address'){	
				if($field['autocompleteField']==1){
					...
					$this->SL['sl_'.$field_id]	=	empty($field['autocompleteSLField'])?false:true;
					$this->CC['cc_'.$field_id]	=	empty($field['autocompleteCCField'])?false:true;
				}
			}
			else{
				continue;
	...
	function bb_gf_footer(){
		...
		jQuery(document).bind('gform_post_render', function(evt,formid){
			...
			var SLs	=	<?php echo json_encode($this->SL); ?>;
			var CCs	=	<?php echo json_encode($this->CC); ?>;
